...
- **Source code reorganized into `core/` + `lib/` structure** — `src/` now has
  three top-level entries: `index.ts` (library API, was `lib.ts`), `bin.ts`
  (CLI binary entry, was `index.ts`), `lib/` (standalone utilities: config,
  utils, logger, ui, cli, browser-cookies), and `core/` (business logic:
  monitor, api/, recorder/). Import paths updated throughout. No behavioral
  changes.

### Fixed

- **`bun run dev` now works after reorg** — the `dev` script was still pointing
  to `src/index.ts` (now the library entry), which has no `main()` function.
  Updated to point to `src/bin.ts`.
